...
class TimeDataset(Dataset):
    def __init__(self, data: ndarray, slide_window: int, slide_stride: int, mode: Literal['train', 'test'], dtype=None):
    ...
    @staticmethod
    def __cut(data: ndarray, slide_window: int, slide_stride: int, mode: Literal['train', 'test'], dtype) -> tuple[Tensor, Tensor, Tensor]:
        if mode == 'test':
            slide_stride = 1
